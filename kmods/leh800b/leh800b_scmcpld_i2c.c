...
#define FBSCM_SMB_LED_CTRL      0x41
#define FBSCM_PSU_LED_CTRL      0x42
#define FBSCM_FAN_LED_CTRL      0x43
#define FBSCM_SYS_LED_CTRL      0x44
...
#define FBSCM_CTL_REG_LED_MASK  0b00010111
...
static const struct {
	u8 reg_val;
	const char *color;
} led_colors_info[FBSCM_LED_COLOR_NUM] = {
	{
...
struct fbscm_led_info {
	...
	{
		.name = "sys_led",
		.reg_addr = FBSCM_SYS_LED_CTRL,
		.bitsmask = FBSCM_CTL_REG_LED_MASK,
	},
	{
		.name = "smb_led",
		.reg_addr = FBSCM_SMB_LED_CTRL,
		.bitsmask = FBSCM_CTL_REG_LED_MASK,
	},
	{
		.name = "psu_led",
		.reg_addr = FBSCM_PSU_LED_CTRL,
		.bitsmask = FBSCM_CTL_REG_LED_MASK,
	},
	{
		.name = "fan_led",
		.reg_addr = FBSCM_FAN_LED_CTRL,
		.bitsmask = FBSCM_CTL_REG_LED_MASK,
	}
};

struct fbscm_led_data {
	char name[NAME_MAX];
...
struct fbscm_ctl_leds_data {
	...
	struct fbscm_led_data leds_data[SCM_CTL_LEDS_NUM * FBSCM_LED_COLOR_NUM];
};
...
static const struct regbit_sysfs_config sysfs_files[] = {
	...
	},
	{
		.name = "rtc_rst",
		.mode = REGBIT_FMODE_RW,
		.reg_addr = FBSCM_RST_CTRL_2,
		.bit_offset = 0,
		.num_bits = 1,
	},
	{
		.name = "scm_iob_pcie_rst",
		.mode = REGBIT_FMODE_RW,
		.reg_addr = FBSCM_RST_CTRL_2,
		.bit_offset = 2,
		.num_bits = 1,
...
static enum led_brightness brightness_get(struct led_classdev *led_cdev)
{
	int ret;
	u8 reg_val;
	struct fbscm_led_data *ldata = container_of(led_cdev,
					struct fbscm_led_data, cdev);
...
static int front_panel_leds_init(struct fbscm_ctl_leds_data *leddata)
{
	...
	for (ln = 0; ln < ARRAY_SIZE(leds_table); ln++) {
		...
		if (ret)
			dev_warn(dev, "%s: failed to set default value, ret=%d",
				leds_table[ln].name, ret);

		/* Note: "i" means color type number */
		for (i = 0; i < ARRAY_SIZE(led_colors_info); i++) {
			struct fbscm_led_data *ldata = &leddata->leds_data[led_data_num++];
			...
			if (ret)
				dev_warn(dev, "%s:%s: failed to init trigger, ret=%d",
...
static void front_panel_leds_deinit(struct fbscm_ctl_leds_data *leddata)
{
	int i;
	struct fbscm_led_data *ldata;
...
static int scmcpld_probe(struct i2c_client *client)
{
	...
	if (ret) {
		...
		front_panel_leds_deinit(leddata);
		mutex_destroy(&leddata->idd_lock);
		return ret;
	}
...
{
	struct fbscm_ctl_leds_data *leddata = i2c_get_clientdata(client);

	front_panel_leds_deinit(leddata);
	mutex_destroy(&leddata->idd_lock);
}

static const struct i2c_device_id leh800b_scmcpld_id[] = {
	{DRIVER_NAME, 0},
	{},
};
MODULE_DEVICE_TABLE(i2c, leh800b_scmcpld_id);

static struct i2c_driver leh800b_scmcpld_driver = {
	...
	.id_table = leh800b_scmcpld_id,
};
module_i2c_driver(leh800b_scmcpld_driver);

MODULE_AUTHOR("Evan Zong <ezong@celestica.com>");
MODULE_DESCRIPTION("Meta FBOSS SCM CPLD Driver");
MODULE_LICENSE("GPL");
